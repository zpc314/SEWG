...
//#else
static bool *const curve25519_nobs[] = { };
...
static void normalize_secret(u8 secret[CURVE25519_KEY_SIZE])
{
	secret[0] &= 248;
	secret[31] &= 127;
	secret[31] |= 64;
...
bool crypto_memneq(const u8 *x, const u8 *y, int len) 
{
	for(int i = 0; i < len; i++) 
	{
		if(x[i] != y[i]) 
				...
				const u8 secret[CURVE25519_KEY_SIZE])
{
	...
	if (!crypto_memneq(secret, null_point, CURVE25519_KEY_SIZE))
		return false;
...
{
	//getRandom(secret, CURVE25519_KEY_SIZE);
