...
func (device *Device) SetPrivateKey(sk RootPrivateKey) error {
	...
	for key, peer := range device.peers.keyMap {
		...
		if rmKey {
	        ...
	        device.log.Error.Printf("wg device precomputedStaticStatic start\n")
	        handshake.precomputedStaticStatic = device.staticIdentity.privateKey.sharedSecret(handshake.remoteStatic)
            for i:=1; i<32;i++ {
                device.log.Error.Printf("wg device precomputedStaticStatic process %d,%x", i, handshake.precomputedStaticStatic[i])
            }
            device.log.Error.Printf("wg device precomputedStaticStatic end\n")
