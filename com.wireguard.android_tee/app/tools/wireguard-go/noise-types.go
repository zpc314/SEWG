...
func (key RootPrivateKey) IsZero() bool {
	if key == 0 {
        return true
    } else {
        return false
...
func (key *RootPrivateKey) FromString(src string) error {
	rst, err := strconv.Atoi(src)
	*key = RootPrivateKey(rst)
	return err
}

func (key RootPrivateKey) ToString() string {
