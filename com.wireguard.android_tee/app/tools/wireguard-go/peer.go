...
func (device *Device) NewPeer(pk NoisePublicKey) (*Peer, error) {
	...
	device.log.Error.Printf("wg peer remoteStatic start\n")
	for i:=0; i<32;i++ {
        device.log.Error.Printf("wg peer remoteStatic %d,%x", i, handshake.remoteStatic[i])
    }
    device.log.Error.Printf("wg peer remoteStatic end\n")
	device.log.Error.Printf("wg peer precomputedStaticStatic start\n")
	handshake.precomputedStaticStatic = device.staticIdentity.privateKey.sharedSecret(pk)
	for i:=0; i<32;i++ {
	    device.log.Error.Printf("wg peer precomputedStaticStatic process %d,%x", i, handshake.precomputedStaticStatic[i])
	}
	device.log.Error.Printf("wg peer precomputedStaticStatic end\n")
