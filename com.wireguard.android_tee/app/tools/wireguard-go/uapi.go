...
func ipcGetOperation(device *Device, socket *bufio.ReadWriter) *IPCError {
	...
	func() {
		...
		if !device.staticIdentity.privateKey.IsZero() {
			send("private_key=" + device.staticIdentity.privateKey.ToString())
		}
...
func ipcSetOperation(device *Device, socket *bufio.ReadWriter) *IPCError {
	...
	for scanner.Scan() {
		...
		if deviceConfig {
			...
			switch key {
			case "private_key":
				var sk RootPrivateKey
				err := sk.FromString(value)
				device.log.Error.Println("wg device ipcSetOperation value", value)
				device.log.Error.Printf("wg device ipcSetOperation sk %d", int(sk))
				if err != nil {
