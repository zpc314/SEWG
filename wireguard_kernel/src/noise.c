...
{
	static_identity->static_private = private_key;
	static_identity->has_identity = Core_GetPublickeyByID(
...
//zpc added
static bool __must_check mix_dh_tee(u8 chaining_key[NOISE_HASH_LEN],
				u8 key[NOISE_SYMMETRIC_KEY_LEN],
				const u32 private,
				const u8 public[NOISE_PUBLIC_KEY_LEN])
{
	...
	if (unlikely(!Core_CurveMul(dh_calculation, public, private)))
		return false;
	kdf(chaining_key, key, NULL, dh_calculation, NOISE_HASH_LEN,
	    NOISE_SYMMETRIC_KEY_LEN, 0, NOISE_PUBLIC_KEY_LEN, chaining_key);
	memzero_explicit(dh_calculation, NOISE_PUBLIC_KEY_LEN);
				      ...
				      struct wg_device *wg)
{
	...
	/* es */
	if (!mix_dh_tee(chaining_key, key, wg->static_identity.static_private, e))
		goto out;
				    ...
				    struct wg_device *wg)
{
	...
	/* se */
	if (!mix_dh_tee(chaining_key, NULL, wg->static_identity.static_private, e))
		goto fail;
